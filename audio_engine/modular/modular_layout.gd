class_name Modular_Layout
extends Resource

@export var modules : Array[Modular_Module]

## Converts itself to a the C++ GDExtension object, which can be processed much faster
func to_voice() -> Modular_Voice:
	var voice := Modular_Voice.new()
	...
	var connection_map : Dictionary[Modular_Connection, int] # Modular_Connection -> index in memory data
...
func to_polyvoice(
	...
	var polyvoice := Modular_Polyvoice.new()
	polyvoice.voices = []
	
	for i in range(voice_count):
		var new_voice : Modular_Voice = to_voice()
...
func to_polyvoices(
	polyvoice_count : int,
	voice_count : int
) -> Array[Modular_Polyvoice]:
	var polyvoices : Array[Modular_Polyvoice]
