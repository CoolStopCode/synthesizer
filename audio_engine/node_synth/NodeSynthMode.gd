...
func key_pressed(index: int) -> void:
	...
	var chord := build_chord(index, bend)
...
func bend_changed() -> void:
	...
	for polyvoice in voice_manager.polyvoices:
		if polyvoice.active:
			voice_manager.bend_polyvoice(polyvoice, build_chord(i, bend), chord_bend_duration)
		i += 1

func build_chord(index: int, bend : Vector3i) -> Chord:
	#match direction:
	...
	match bend:
		Vector3i(0, 0, 0): return scale.triad(index)
		
		Vector3i(1, 0, 0): return scale.triad(index).shift_octave(1)
		Vector3i(0, 1, 0): return scale.root_note(index)
		Vector3i(0, 0, 1): return scale.triad(index).shift_octave(-1)
		
		Vector3i(1, 1, 0): return scale.cinimatic(index)
		Vector3i(0, 1, 1): return scale.triad(index).invert(1, 1)
		Vector3i(1, 0, 1): return scale.triad_parallel(index)
		
		Vector3i(1, 1, 1): return scale.seventh(index)
